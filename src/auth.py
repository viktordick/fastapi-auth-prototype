...
LoginCookieDep = Annotated[
    ...
    Depends(
        APIKeyCookie(
            name="__login", description="Generated by /login", auto_error=False
        )
    ),
]
...
def user(session: SessionDep, cookie: LoginCookieDep) -> Optional[AppUser]:
...
UserDep = Annotated[Optional[AppUser], Depends(user)]


@app.put("/admin/generate_user")
async def generate_user(username: str, password: str, session: SessionDep):
    """
    We would not actually allow this, at least not without first checking elevated permissions
    """
